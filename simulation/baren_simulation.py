...
def data_connect( data1, data2, horce_num1, horce_num2 ):
    result = []

    if horce_num1 < horce_num2:
        result.extend( data1 )
        result.extend( data2 )
    else:
        result.extend( data2 )
...
def main():
    ...
    for race_id in tqdm( data.keys() ):
        ...
        horce_id_1 = sort_result[0]["horce_id"]
        data_1 = rank_dist_data[race_id][horce_id_1]["data"]
        predict_rank_dist = []

        for i in range( 1, len( sort_result ) ):
            horce_num_2 = sort_result[i]["horce_num"]
            horce_id_2 = sort_result[i]["horce_id"]
            data_2 = rank_dist_data[race_id][horce_id_2]["data"]
            use_data = data_connect( data_1, data_2, horce_num_1, horce_num_2 )
            rank_dist = rank_dist_model.predict( np.array( [ use_data ] ) )[0]
            predict_rank_dist.append( { "horce_num": horce_num_2, "rank_dist": rank_dist, "rank": sort_result[i]["rank"] } )

        predict_rank_dist = sorted( predict_rank_dist, key=lambda x:x["rank_dist"] )

        for i in range( 0, 3 ):
            rank_2 = predict_rank_dist[i]["rank"]
            horce_num_2 = predict_rank_dist[i]["horce_num"]
            baren_odds = baren_odds_get( horce_num_1, horce_num_2 )

            if baren_odds == -1:
                continue

            if 4 < predict_rank_dist[i]["rank_dist"]:
                continue
